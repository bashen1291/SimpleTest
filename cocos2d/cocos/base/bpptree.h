...
protected:
    ...
    union memory_node_t
    {
        uint8_t space[config_t::memory_block_size];
    };
    typedef typename allocator_type::template rebind<memory_node_t>::other node_allocator_t;
    template<class lldb_leaf_t, class lldb_value_t> struct root_node_type : public node_t, public key_compare, public node_allocator_t, public status_t
    {
        template<class any_key_compare, class any_allocator_t> root_node_type(any_key_compare &&comp, any_allocator_t &&alloc) : key_compare(std::forward<any_key_compare>(comp)), node_allocator_t(std::forward<any_allocator_t>(alloc)), status_t()
        {
    ...
    typedef root_node_type<leaf_node_t, value_type> root_node_t;
    struct key_stack_t
    {
        struct
        {
            uint8_t space[sizeof(key_type)];
        } key_pod;
        key_stack_t()
        {
        }
